...
public class SpellNexusMenu extends AbstractContainerMenu {
    ...
    public SpellNexusMenu(int pContainerId, Inventory inv, BlockEntity blockEntity, ContainerData data)
    {
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            this.addSlot(new WandSlot(iItemHandler, 0, 80, 15));
            this.addSlot(new SpellHologramSlot(iItemHandler, 1, 80, -31));
            this.addSlot(new SpellHologramSlot(iItemHandler, 2, 80+48, 18));
            this.addSlot(new SpellHologramSlot(iItemHandler, 3, 80, 66));
            this.addSlot(new SpellHologramSlot(iItemHandler, 4, 80-48, 18));
        });
    ...
    private void addPlayerInventory(Inventory pPlayerInv)
    {
        for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 9; ++j)
            {
                this.addSlot(new Slot(pPlayerInv, j + i*9 + 9, 8 + j*18, 111 + i*18));
            }
        }
    }
    ...
    private void addPlayerHotbar(Inventory pPlayerInv)
    {
        for (int i = 0; i < 9; i++)
        {
            this.addSlot(new Slot(pPlayerInv, i, 8 + i*18, 169));
        }
    }
